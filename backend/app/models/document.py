...
class Document(Base):
    ...
    # Computed property for text length (PRD §5.1 Optimization)
    # Allows fetching length without loading the full Text column
    extracted_text_length: Mapped[int] = column_property(
        func.length(func.coalesce(extracted_text, ""))
    )
