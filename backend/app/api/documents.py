...
async def upload_document(
    ...
    # Validate file size (FastAPI 0.115.6+ provides file.size)
    if hasattr(file, "size") and file.size and file.size > settings.max_upload_size_bytes:
    ...
    try:
        ...
        # Don't leak internal path or error details in public response
        doc.error_message = f"Failed to save file: {str(e)}"
        await db.flush()
