...
async def upload_document(
    ...
    # Enforce file size limit (PRD §6.4)
    # UploadFile.size is available in Starlette/FastAPI, but we use a fallback if needed.
    file_size = getattr(file, "size", None)
    if file_size is None:
        # Fallback for older versions: seek to end to find size
        file.file.seek(0, 2)
