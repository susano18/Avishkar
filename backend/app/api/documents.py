...
async def upload_document(
    ...
    # Validate file size
    # Some FastAPI/Starlette versions don't populate 'file.size' reliably.
    # Seek to end to get actual byte count.
    file.file.seek(0, 2)
    ...
    try:
        ...
        # Log the detailed error but keep the API response generic to avoid leakage
        logger.error("file_save_failed", doc_id=doc.id, error=str(e))
        doc.error_message = "Internal error while saving file."
        await db.flush()
        raise HTTPException(
            status_code=500,
    ...
    try:
        ...
        doc.status = ProcessingStatus.FAILED
        # Sanitize error message stored in DB and logged, avoiding system path leakage
        doc.error_message = "Content extraction failed."
