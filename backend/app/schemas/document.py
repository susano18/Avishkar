...
class DocumentSlimResponse(BaseModel):
    """Schema for a document's metadata (excludes large text fields)."""
...
class DocumentResponse(DocumentSlimResponse):
    """Schema for a single document's full details (includes extracted text)."""
...
class DocumentListResponse(BaseModel):
    ...
    documents: list[DocumentSlimResponse] = Field(
        ..., description="List of document records (slim versions)."
    )
